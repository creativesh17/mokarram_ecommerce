...
const actions = {
    ...
    [`fetch_${store_prefix}_add_to_home_cat`]: async function ({state, commit }, id) {
        let res = await axios.post("/category/add-to-home-cat",{id})
        if(res.data){
            window.s_alert("added to home category");
        }else{
            window.s_alert("removed from home category");
        }
        return res.data;
    },
    [`fetch_${store_prefix}_add_to_nav_cat`]: async function ({state, commit }, id) {
        let res = await axios.post("/category/add-to-nav-cat",{id})
        if(res.data){
            window.s_alert("added to nav category");
        }else{
            window.s_alert("removed from nav category");
        }
        return res.data;
    },
