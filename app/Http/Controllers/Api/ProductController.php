...
class ProductController extends Controller
{
    ...
    public function products()
    {
        $key = request('key');
        if ($key == 'storage') {
            $data = Product::where(function ($q) use ($key) {
                return $q->where('product_name', $key)
                    ->orWhere('product_name', 'LIKE', "%HDD%")
                    ->orWhere('product_name', 'LIKE', "%hdd%")
                    ->orWhere('search_keywords', 'LIKE', "%SDD%")
                    ->orWhere('search_keywords', 'LIKE', "%sdd%")
                    ->orWhere('search_keywords', 'LIKE', "%disk%")
                    ->orWhere('search_keywords', 'LIKE', "%Disk%");
            });
        } else {
            $data = Product::where(function ($q) use ($key) {
