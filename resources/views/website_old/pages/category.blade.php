    ...
    <section>
        ...
        <div class="container">
            ...
            <div class="child_list" id="categories_and_brands">
                @foreach($category->child()->where('status',1)->get() as $child)
                    <a href="{{ url($child->url) }}">{{$child->name}}</a>
                @endforeach
                @foreach($category->category_tag as $tag)
                    <a href="{{ url($tag->url) }}">{{$tag->title}}</a>
                @endforeach
            </div>
        </div>
        @endif
...
<div class="container">
    <div class="product_list">
        ...
        <div >
            ...
            @if ( $category && strlen($category->description) > 30)
                <div class="category_page_top">
                    <div class="description">
