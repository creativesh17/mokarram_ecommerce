...
class CategoryController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $data->url = save_website_url($data->url, 'categories', $data->id);
        $data->save();
    ...
    public function add_to_home_cat()
    {
        ...
        $id = request()->id;
        $category = Category::find($id);
        if($category->is_home_category){
            $category->is_home_category = 0;
            $category->save();
            return response()->json(0);
        }else{
            $category->is_home_category = 1;
            $category->save();
            return response()->json('success');
        }
    }
    ...
    public function add_to_nav_cat()
    {
        ...
        $id = request()->id;
        $category = Category::find($id);
        if($category->is_nav_category){
            $category->is_nav_category = 0;
            $category->save();
            return response()->json(0);
        }else{
            $category->is_nav_category = 1;
            $category->save();
            return response()->json('success');
        }
    }
