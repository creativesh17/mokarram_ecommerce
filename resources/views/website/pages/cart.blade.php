@extends('website.layouts.app', [ 'meta' => [ 'title' => 'Cart', ], ]) @php $subtotal = 0; $shippingCharge = 50; @endphp @section('content') @include('website.layouts.partials.breadcrumb')
            ...
            <div class="col-lg-8">
                ...
                <div class="table-cart mCustomScrollbar _mCS_1 mCS_no_scrollbar">
                    <div id="mCSB_1" class="mCustomScrollBox mCS-light mCSB_horizontal mCSB_inside" style="max-height: none;" tabindex="0">
                        <div id="mCSB_1_container" class="mCSB_container mCS_x_hidden mCS_no_scrollbar_x" style="position: relative; top: 0px; left: 0px; width: 464px; min-width: 100%; overflow-x: inherit;" dir="ltr">
                            <form action="{{ url('/cart/update') }}" method="POST">
                                ...
                                <table>
                                    ...
                                    <tbody>
                                        @foreach ($cartItems as $key => $product) @php $subtotal += $product->qty * $product->price; @endphp
                                        <input type="hidden" name="product[{{ $key }}][rowId]" value="{{ $product->rowId }}" />
                                        <input type="hidden" name="product[{{ $key }}][id]" value="{{ $product->id }}" />
                                        <tr>
                                            <td>
                                                <div class="d-flex gap-3">
                                                    ...
                                                    <div>
                                                        <div class="name-product">
                                                            {{ $product->name }} @if($product->options->v_titles) @foreach ($product->options->v_titles as $title) - {{ $title }} @endforeach @endif
                                                        </div>
